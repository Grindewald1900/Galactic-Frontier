...
> 上级约束：`02-core-product-design.md` §7.3 / §7.4 / §16.1 / §20 / §21 / §22；成长经验见 `19-characters-and-progression.md` 自动成长  
> 关联：`11-deck-and-occupation.md`（占用 / `PausedCap`）、`12-auto-battle.md`（无场景结算）、`13-region-and-ship.md`（通关后刷取解锁、舰船模块）  
> 更新日期：2026-09-02  
> 变更：v1.2 **废止 Offline Yield Ratio 打折**（原开局 50%）。有效离线时间内资源、卡牌经验、舰长经验均为 100%；舰船升级拉长 cap、扩仓、自动补给与并行。上线须展示关键升级。v1.1 曾增补 Yield Ratio，已被本版取代。
...
- 有效离线时间内资源与经验 **足额结算**；用舰船模块拉长 cap、扩仓、自动补给，而不是打折产出；
- 上线结算须突出卡牌 / 舰长关键升级与解锁（见 `19`）；
...
| **离线结算完整度** | 有效离线时间内相对在线同周期为 **1.0**；不再使用 Yield Ratio 打折 |
...
2. 离线收益时间属于**成长内容**，须分阶段解锁/提升；有效离线时间内资源、卡牌经验、舰长经验按 **100%** 结算（受仓库、补给约束）。**不得**用离线效率折扣惩罚玩家。  
...
| 条件 | `+秒` | 累计示例（自 2h 起） |
| --- | --- | --- |
| 舰长等级 ≥ 5 | +1800（+0.5h） | 2.5h |
| 舰长等级 ≥ 10 | +1800 | 3.0h |
| 舰长等级 ≥ 15 | +3600（+1h） | 4.0h |
| 舰长等级 ≥ 20 | +3600 | 5.0h |
| 主线章节 1 通关 | +3600 | … |
| 主线章节 2 通关 | +3600 | … |
| 主线章节 3 通关 | +7200（+2h） | … |
...
### 4.2b 离线结算完整度（取代 Yield Ratio）

> `02` v1.1 / `19` 自动成长 §10：有效离线时间内 **不打折**。原 v1.1「开局 50% Yield Ratio」作废；已实现的比例表应改为恒 `1.0` 或删除乘区。

| 项目 | 规则 |
| --- | --- |
| 在线挂机产出 | **100%** |
| 有效离线产出 | **100%**（资源、卡牌经验、舰长经验） |
| 约束 | 仓库容量、补给、离线 cap；满仓 / 缺补给 → `PausedBlock`，不打折继续空转 |
| 不作用 | 已写入仓库的历史收益；耐久仍按场次/周期满计 |
...
| 回上线弹窗 | 展示 raw/credited 时长、是否触顶；**按行动列出资源**；**按角色列出战斗/专业技能升级与解锁**；舰长升级与规模解锁；主按钮「领取全部」。禁止只显示「铁矿 ×200」 |
...
- 有效离线时间内资源、卡牌经验、舰长经验均为 **100%**（不再使用 50% Yield Ratio）
- 开局离线上限为 2 小时；成长后可提升，硬顶 ≤ 24 小时
- 上线弹窗须列出关键升级 / 解锁，而不仅是资源清单
...
- 客户端时间回拨不导致资源被扣（后续加固）
- EditMode：cap 与耐久基础公式；离线乘区恒为 1.0
