...
> 更新日期：2026-08-30  
> 变更：拍板 **两层航网**、五种通航状态、环带+网状路线、航线资产化；目标从「打到中心」改为「重新编织通往中心的航网」。
...
M1 **不实现**宇宙航网与五种状态；只在**单个星域内部**提供 2D 团块 + 雷达 + 巡航。实现状态见 PRODUCT-STATUS。
...
| --- | --- |
| 舰船坐标 | `navX/navY`；存档权威 |
| 雷达 | `radarRange` 由扫描模块推导；移动才揭示 |
| 迷雾 | 未揭示且未海图的点不可见或仅噪点 |
| 巡航 Cruise | 域内移动；耗时、耗能源；可离线推进 |
| 已知点 | `knownBodyIds` |
| 跃迁 Jump / 信标 | **M3**；M1 不做跨域 |
...
visibleBodies = bodies where distance(ship, body) <= radarRange
             ...
             OR body.id ∈ knownBodyIds
```

### 12.2 航行时间与反时间墙

继承 `14`：航行写入 `TransitAction`；离线计入 credited 时间；允许材料/信用加速短途；禁止纯时间墙。航行中已启动的采集/制造可继续。
...
中枢交互或 NPC 购图 → 揭示该星域高比例坐标。解锁后常亮，仍须航行抵达才能登陆。主星坐标在探索度 50% 前不可购图直达（可任务预告）。
...
| **M2** | 海图/中枢 + 域内航网状态雏形 | 信息经济；熵雾覆盖/已定位 |
| **M3** | 第二星域 + 信标跃迁 + 宇宙层边 | 外缘→开拓带；路线差试点 |
...
服务方向：`NavigationService`（M1 巡航）+ 后继 `GridService`（节点状态、边升级、虫洞）。
