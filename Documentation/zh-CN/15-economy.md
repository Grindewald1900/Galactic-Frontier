...
| 仓库 | 按品质筛选/着色；装备显示品质徽章；**P6**：正式名称、来源/用途、缺口筛选（[22](22-ux-loop-and-ia.md)） |
| 拆解/再加工 | 二次确认；显示返还预览 |

#### P6 制造三栏 UI（Wave 1；非 ProductionLine 实体）

| 栏 | 内容 |
| --- | --- |
| 左 | 进行中队列 + 暂停原因 |
| 中 | 配方列表 + 材料行（缺口 `ShortageJump`） |
| 右 | 产出、耗时、品质区间、负责人占位 |

Wave 1 **不做**：自动采购、库存目标、持续生产实体。规则实体仍见 §4.10（后 MVP）。
