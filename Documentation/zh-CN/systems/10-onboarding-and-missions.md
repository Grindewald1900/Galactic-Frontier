...
> 更新日期：2026-08-22  
> 变更：v1.2 — 对齐 `00` §10 目标故事弧（首战教训→制造→再战→订单）；当前 5 步链为系统教学最小集。
...
2. **设定目标**（`00` §10）：在首圈让玩家经历「首战教训 → 制造针对性装备 → 再战改善 → 接开拓局订单指向下区」的策略闭环；当前实现以 5 步系统教学为最小集，二次战斗与订单可渐进增强；  
3. 进度可**持久化**，重进游戏不丢；  
4. Bridge / Missions 与相关页有**软引导**（高亮下一步入口），不强弹强制教程；  
5. **Solo 不出现玩家市场**步骤或文案；  
6. 为后续区域包装（P5.3）与角色扩容（P5.1）留挂钩，但不依赖它们完成主链。
