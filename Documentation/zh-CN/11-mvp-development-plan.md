...
> 更新日期：2026-08-09  
> 变更摘要：P1 完成（多卡组 UI、ActionScheduler、备用预设）；下一阶段 P2 区域推进。
本文档回答三件事：
...
| 多行动并行 + 行动队列 | 部分实现 | P1 并行上限 + `ActionScheduler`；采集/制造周期结算仍属 P3 |
...
1. ~~**P0.1–P0.5** 稳基线~~ **已完成**  
2. ~~**P1** 多卡组与占用~~ **已完成**（数据 / UI / 调度器 / 备用预设）  
3. **P2.1** 区域通关标记与 Explore 解锁  
4. **P2.3** 遭遇表替换 Dev 敌人  
5. **P3.1** 资源表接入仓库  
