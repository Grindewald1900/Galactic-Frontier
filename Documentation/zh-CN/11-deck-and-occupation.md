...
> 关联成长：`19-characters-and-progression.md`（任务分配即培养成本）  
> 更新日期：2026-09-02  
> 变更：v1.3 解锁表「玩家等级」更名为 **舰长等级**；占用即自动成长的操作成本。
...
把角色变成**有限运营资源**：玩家必须在主线推进、挂机刷取、采集、制造等行动之间分配卡牌，而不是把最强阵容同时铺满所有系统。占用时间就是培养成本——经验自动涨，玩家只决定「让谁做什么」（见 `19`）。
...
| --- | --- | --- |
| 卡组槽 #3 | **舰长等级** ≥ 5 **或** 主线章节 1 通关 | 常用：战斗 + 采集 + 制造 |
| 卡组槽 #4 | **舰长等级** ≥ 10 **或** 舰船等级 ≥ 2 | |
| 卡组槽 #5 | 主线章节 2 通关 **或** 舰船设施「调度中枢」Lv.1 | |
| 卡组槽 #6 | 舰船等级 ≥ 4 **或** 研究「多编队协议」完成 | |
| 并行上限 3 | **舰长等级** ≥ 8 **或** 舰船设施「自动化核心」Lv.1 | |
...
- 开局 2 个卡组槽可用，并行上限为 2（`DeckStateFactory` + 单测）  
- 每卡组最多 5 人；同卡组内不可重复 `cardId`（`DeckRules.TryAssignSlot`）  
- 同一 `cardId` 不能同时处于两个 Running 卡组  
- 不同 `cardId` 的同名角色可同时 Running  
- 未运行卡组可共享同一 `cardId` 预设且不占用  
- 启动冲突时有明确失败原因（`DeckCommandError` / `ConflictCardIds`）  
- 停止后占用立即解除，可立刻改打其他行动  
- 主动停止不扣除已入包资源、不加长时间锁定（当前仅清状态；资源惩罚属后续生产系统）  
- 旧存档仅有 `LineupPosition` 时可迁移出默认战斗卡组（Migrator 1→2 / `EnsureLoaded`）  
- 领域规则具备 EditMode 测试（冲突启动、并行上限、停止解锁、同名并行、Running 不可编辑）  
- Formation / Bridge 多卡组 UI 与解锁提示（P1.2）  
- 通用行动调度器（P1.4）  
- 备用卡组可在 UI 中编辑且不占用（P1.5）
...
## P6 Formation UI（见 [22-ux-loop-and-ia.md](22-ux-loop-and-ia.md)）

| 项 | 规则 |
| --- | --- |
| 阵型 | 中央区展示 **前排 2 + 后排 3** 槽位（展示层；战斗站位规则仍以 [12](12-auto-battle.md) 为准，未改前为 display-only） |
| 选择 | 点击角色显示职业、技能、占用、替换后战力/关键属性变化 |
| 停止 | 按钮文案须含具体行动名，如「停止外缘港采集」 |
| 右侧 | 队伍优劣势摘要（破甲/生存/治疗等）与预估胜率占位 |
| 底部 | 重复卡组区改为「备选角色」列表 |
