...
> 契约：见 [06-data-and-save.md](06-data-and-save.md)。`DefaultProperty.isDebug` 仍只控制明文/Base64，不打开 FakeData。
...
- 自动化流水线、舰队、星图 M3+（宇宙层 / 信标跃迁）（见 PRODUCT-STATUS §5）。M2 域内航网雏形已落地。
- 多个公开 Inspector 字段缺少统一命名与空值验证。
- `CardEntity` 同时承担持久化模型、属性计算和事件通知，职责较重。

## UX / P6 技术债（见 [22-ux-loop-and-ia.md](22-ux-loop-and-ia.md)）

| 项 | 现状 | 目标 |
| --- | --- | --- |
| 一级导航 | 12+ 平铺项 | 五组 + 子 Tab |
| 舰桥主页 | 数字汇总 + 同级 CTA | 主目标 + 瓶颈 + 唯一 Primary |
| 缺口跳转 | 仅 ItemTooltip 悬停 | 全屏 ShortageJump 行 |
| 星图 | 地图约 1/3 宽 | ≥70% + 四轴状态 + 条件 CTA |
| 制造 | 黑列表 + 空详情 | 三栏 + 队列 |
| 编队 | 中央空白 | 2+3 阵型 |
| 仓库/商店 | 调试感列表 | 正式名 + 用途 + 需求筛选 |
| 招募 | 空白页 | 池视觉 + 保底 + 券来源 |
| 按钮语义 | 多色混用 | NexusButtonRole |
| 对比度 | 锁定项过暗、小字灰 | Wave 3 回归 |
