...
> 关联：`14-deck-and-occupation.md`（占用 / `PausedCap`）、`15-auto-battle.md`（无场景结算）、`16-region-and-ship.md`（通关后刷取解锁、舰船模块）  
> 更新日期：2026-08-10  
...
- 用**离线收益比例**（开局低于在线）激励舰船模块成长，同时保证离线仍有可观回报；
...
- 多卡组占用与并行上限本身 → `14-deck-and-occupation.md`
- 单场战斗回合规则与战前策略 → `15-auto-battle.md`
- 区域解锁与舰船硬门 → `16-region-and-ship.md`
- 生产链配方、品质档位 → `18-production-and-quality.md`
- 耐久损耗与维修公式 → `19-durability-and-repair.md`
- 全服市场撮合 → `20-market-and-card-trade.md`
...
| **离线收益比例（Offline Yield Ratio）** | 离线结算时相对「同等在线周期满额产出」的乘数；开局 **0.5** |
...
2. 离线收益时间属于**成长内容**，须分阶段解锁/提升；**离线收益比例**开局 50%，主要随舰船成长提升。  
...
### 4.2b 离线收益比例（Yield Ratio）

> 核心设计 v0.4：离线产出相对在线打折；与 Offline Cap **独立**计算。

| 项目 | MVP 默认 |
| --- | --- |
| 在线挂机产出 | **100%**（比例 = 1.0） |
| 开局离线比例 | **50%**（0.50） |
| MVP 软顶 | **90%**（0.90）；后续可开放至 100% |
| 硬顶 | **100%** |
| 作用对象 | 资源数量、信用、可堆叠掉落；**经验是否打折可配置，默认与资源同比例** |
| 不作用 | 已写入仓库的历史收益；耐久损耗次数（仍按场次/周期满计，见耐久文档） |

#### 公式（语义）

```text
offlineYieldRatio = clamp(
    baseRatio                                          // 0.50
    + sum(shipLevelRatioBonuses)
    + sum(shipModuleRatioBonuses)                      // 生命维持 / 自动化等模块为主
    + sum(researchRatioBonuses),                       // 可选，宜小
    0.50, hardRatioCap)                                // 不低于开局底；不超过硬顶

offlineReward = floor(onlineEquivalentReward * offlineYieldRatio)
```

周期数仍由 `credited` 时长决定；**先算应产出，再乘比例**。

#### 默认比例表（可配置 `OfflineYieldRatioTable`）
...
配表目标：MVP 首圈合理养成后达到约 **75%–90%**，使舰船模块对离线体验有明显体感。
...
| 奖励 | 经验/信用/掉落表引用（分区表见 `24-sector-and-region-content.md` §9）；进入 **Pending Loot**（或配置为直接入仓，见 §4.7） |
...
- `OfflineYieldRatioTable` 各档比例（开局 0.50；舰船/模块加点）  
...
- 卡组占用：`14-deck-and-occupation.md`（`PausedCap`、停止低惩罚）  
- 战斗：`15-auto-battle.md` §8（挂机接口 / Resolver）  
- 区域：`16-region-and-ship.md` §4.6（通关后刷取）  
- 开发计划：`13-mvp-development-plan.md` P2.4 / P3.6  
- 已知问题：`10-known-issues.md`（FakeData、战斗返回）
