...
public class RadarSystem : MonoBehaviour
{
    ...
    private List<float> circleRadii = new List<float>(); // 存储圆环半径
    private List<PlanetEntity> planetEntities = new List<PlanetEntity>(); // 存储星球
    private List<PlanetRadarItem> planetRadarItems = new List<PlanetRadarItem>(); // 存储星球 RadarItem
    ...
    void GeneratePlanetsOnRing(float radius)
    {
        ...
        List<float> usedAngles = new List<float>(); // 存储已使用的角度
    ...
    {
        List<string> planetNames = new List<string>() { "Planet_1", "Planet_2", "Planet_3", "Planet_4", "Planet_5", "Planet_6" };
        int randomIndex = Random.Range(0, planetNames.Count);
        PlanetEntity planetEntity = new PlanetEntity("Planet_1", "Level 1", "Planet", "Description 1");
        planetEntity.planetName = "Planet " + currentIndex;
        planetEntity.planetDescription = "Description " + randomIndex;
        planetEntity.planetLevel = "Level " + randomIndex;
        planetEntity.backgroundSprite = "Planet_" + randomIndex;
        return planetEntity;
    }
}